...
array<double, VERTEX_COUNT> tree_values, tree_values_spend, tree_add, tree_spend, tree_factor;

void Init() {
    tree_values.fill(0);
    tree_values_spend.fill(0);
    tree_add.fill(0);
    tree_spend.fill(0);
    tree_factor.fill(1);
}
...
void Push(size_t v, size_t l, size_t r) {
    for (size_t w = v * 2; w <= v * 2 + 1; ++w) {
        if (w < VERTEX_COUNT) {
            ...
            tree_values_spend[w] += tree_spend[v] * (r - l) / 2;
        }
    }
    tree_factor[v] = 1;
    tree_add[v] = 0;
...
double ComputeSum(size_t v, size_t l, size_t r, size_t ql, size_t qr) {
    ...
    Push(v, l, r);
    if (ql <= l && r <= qr) {
        return tree_values[v] - tree_values_spend[v];
    }
...
void Add(size_t v, size_t l, size_t r, size_t ql, size_t qr, double value) {
    ...
    tree_values_spend[v] =
            (v * 2 < VERTEX_COUNT ? tree_values_spend[v * 2] : 0)
            + (v * 2 + 1 < VERTEX_COUNT ? tree_values_spend[v * 2 + 1] : 0);
}
...
void Multiply(size_t v, size_t l, size_t r, size_t ql, size_t qr, double coef) {
    if (v >= VERTEX_COUNT || qr <= l || r <= ql) {
        return;
    }
    Push(v, l, r);
    ...
    Multiply(v * 2, l, (l + r) / 2, ql, qr, coef);
    Multiply(v * 2 + 1, (l + r) / 2, r, ql, qr, coef);
    tree_values[v] =
            (v * 2 < VERTEX_COUNT ? tree_values[v * 2] : 0)
            + (v * 2 + 1 < VERTEX_COUNT ? tree_values[v * 2 + 1] : 0);
    tree_values_spend[v] =
...
int main() {
    ...
    for (int i = 0; i < q; ++i) {
        ...
        if (query_type == "ComputeIncome") {
            ...
            double value;
            cin >> value;
            double coef = (100.0 - value) / 100.0;
            Multiply(1, 0, DAY_COUNT_P2, idx_from, idx_to, coef);
