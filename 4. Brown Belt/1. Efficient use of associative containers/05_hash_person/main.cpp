...
struct AddressHasher {
    size_t operator()(const Address &a) const {
        const size_t coef = 89;
        const hash<int> int_hash;
        const hash<string> string_hash;

        size_t h1 = string_hash(a.city);
        size_t h2 = string_hash(a.street);
        size_t h3 = int_hash(a.building);
        return (h1 * coef * coef + h2 * coef + h3);
    }
};

struct PersonHasher {
    size_t operator()(const Person &p) const {
        const size_t coef = 67;
        const hash<int> int_hash;
        const hash<double> double_hash;
        const hash<string> string_hash;
        const AddressHasher address_hasher;

        size_t h1 = string_hash(p.name);
        size_t h2 = int_hash(p.height);
        size_t h3 = double_hash(p.weight);
        size_t h4 = address_hasher(p.address);
        return (h1 * coef * coef * coef + h2 * coef * coef + h3 * coef + h4);
    }
};
