...
public:
    ...
    static void PerformMakeBaseRequests(std::istream &input = std::cin);
...
private:
    ...
    std::shared_ptr<YellowPagesDB> yellow_pages_database_;
    std::shared_ptr<Map::MapDataBase> map_database_;

    void build_yellow_pages_database(const Json::Node &db);
    ...
    void perform_read_base_requests(
            const std::vector<Json::Node> &post_requests) const;

    static BusStop read_stop(const Json::Node &request);

    std::pair<std::string, std::shared_ptr<IRouteInfo>> read_bus(const Json::Node &request) const;
    ...
    void perform_find_companies_query(const Json::Node &,
                                      std::map<std::string, std::shared_ptr<Json::JsonWriter>> &) const;
};
