...
struct CompaniesResponse : public Response {
    ...
    explicit CompaniesResponse(std::vector<YellowPages::Company const *> vec) : companies(std::move(vec)) {};
...
void PrintResponses(const std::vector<JsonResponse> &, std::ostream &os = std::cout);
