...
Signals GenerateSignals() {
    Signals signals;
    { // Signal #1 with address, name, and two phone numbers
        Signal signal;
    ...
    { // Signal #2 with name, two phone numbers, and working hours
        Signal signal;
        signal.set_provider_id(271828); // this provider will have the same priority as in the first signal
        auto &company = *signal.mutable_company();
        { // the same name as in the first signal
            auto &name = *company.add_names();
            name.set_value("Рога и копыта");
            name.set_type(Name::MAIN);
        }
        ...
        { // this phone is the same as in the first signal
            auto &phone = *company.add_phones();
            phone.set_formatted("+7 (555) 765-43-21");
            phone.set_type(Phone::FAX);
            phone.set_country_code("7");
    ...
    { // Signal #3 with an address, name, and two websites.
        Signal signal;
        signal.set_provider_id(100500);  // this provider will have lower priority than the previous two
        auto &company = *signal.mutable_company();
        // the address is different from the address in the first signal because it does not include coordinates
