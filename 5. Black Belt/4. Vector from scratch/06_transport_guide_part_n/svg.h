...
namespace Svg {
    ...
    public:
        Rect &SetPointXY(Point newPoint) {
            point = newPoint;
            return *this;
        }
        ...
        void Render(std::ostream &out) const override {
            out << "<rect "
                << "x=\\\"" << point.x << R"(\" y=\")" << point.y << R"(\" width=\")"
                << std::fixed << std::setprecision(6) << width << R"(\" height=\")"
                << std::fixed << std::setprecision(6) << height << "\\\" ";
            PathProps::Render(out);
            out << " />";
        }
    ...
    public:
        ...
        void Add(Rect object) {
            objects.push_back(std::make_unique<Rect>(std::move(object)));
        }
