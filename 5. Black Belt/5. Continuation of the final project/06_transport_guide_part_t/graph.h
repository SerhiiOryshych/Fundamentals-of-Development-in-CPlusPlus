...
namespace Graph {
    ...
    template<typename Weight>
    typename DirectedWeightedGraph<Weight>::IncidenceEdgesRange
    DirectedWeightedGraph<Weight>::GetIncidenceList(VertexID vertex_id) const {
        const auto &es = incidence_lists[vertex_id];
        return {es.begin(), es.end()};
    }

    template<typename Weight>
    DirectedWeightedGraph<Weight>::DirectedWeightedGraph(const RouterProto::Router &router_mes) : incidence_lists(
            router_mes.vertexes().size() * 2) {
