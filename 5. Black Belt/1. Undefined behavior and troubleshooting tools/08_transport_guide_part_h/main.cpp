...
public:
    ...
    void GetMapInfoJson(const MapInfoQuery &query,
        ...
        const auto lat_lng_to_x_y = [&min_max, &zoom_coef,
                &render_settings](LatLng p) {
            return Point{(p.lng - min_max.first.first) * zoom_coef +
                         render_settings.padding,
                         (min_max.second.second - p.lat) * zoom_coef +
                         render_settings.padding};
        };
        ...
        for (const auto &[name, route]: bus_routes) {
            ...
            for (const auto &stop: route.stops) {
                const auto point = lat_lng_to_x_y(stops[stop]);
                polyline.AddPoint(point);
            }

            if (!route.is_roundtrip) {
                for (int i = route.stops.size() - 2; i >= 0; i--) {
                    const auto point = lat_lng_to_x_y(stops[route.stops[i]]);
                    polyline.AddPoint(point);
                }
            }
        ...
        for (const auto &[name, route]: bus_routes) {
            ...
            add_stop(lat_lng_to_x_y(stops[route.stops[0]]));
            if (!route.is_roundtrip &&
                route.stops[0] != route.stops[route.stops.size() - 1]) {
                add_stop(lat_lng_to_x_y(stops[route.stops[route.stops.size() - 1]]));
            }
        ...
        for (const auto &stop: stops) {
            const auto point = lat_lng_to_x_y(stop.second);
            svg.Add(Svg::Circle{}
                            .SetCenter(point)
