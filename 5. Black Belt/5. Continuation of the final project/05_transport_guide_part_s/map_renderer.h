...
public:
    ...
    void Serialize(TCProto::MapRenderer &proto, YellowPages::Database &yellow_pages_db);

    static std::unique_ptr<MapRenderer>
    Deserialize(const TCProto::MapRenderer &proto, const YellowPages::Database &yellow_pages_db);

    [[nodiscard]] Svg::Document Render() const;

    [[nodiscard]] Svg::Document RenderRoute(Svg::Document whole_map, const TransportRouter::RouteInfo &route) const;

private:
    MapRenderer() = default;

    RenderSettings render_settings_;
    std::map<std::string, Svg::Point> stop_points_;
    std::map<int, Svg::Point> company_points_;
    ...
    void RenderCompanyLines(Svg::Document &svg) const;

    void RenderCompanyPoints(Svg::Document &svg) const;

    void RenderCompanyLabels(Svg::Document &svg) const;
    ...
    void RenderRouteCompanyPoint(Svg::Document &svg, const TransportRouter::RouteInfo &route) const;
    ...
    void RenderRouteStopLabels(Svg::Document &svg, const TransportRouter::RouteInfo &route_without_walk_item) const;
