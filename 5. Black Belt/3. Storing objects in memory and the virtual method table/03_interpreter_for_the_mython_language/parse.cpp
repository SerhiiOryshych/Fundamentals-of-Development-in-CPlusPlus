...
private:
    ...
    unique_ptr<Ast::Statement> ParseClassDefinition() {
        ...
        if (lexer.CurrentToken() == '(') {
            ...
            if (auto it = declared_classes.find(name); it == declared_classes.end()) {
                throw ParseError("Base class " + name + " not found for class " +
                                 class_name);
            } else {
        ...
        auto [it, inserted] = declared_classes.insert(
                {class_name, ObjectHolder::Own(Runtime::Class(
                        class_name, std::move(methods), base_class))});
        ...
        if (lexer.CurrentToken() == '=') {
            ...
            if (id_list.empty()) {
                ...
                return make_unique<Ast::FieldAssignment>(
                        Ast::VariableValue{std::move(id_list)}, std::move(last_name),
                        ParseTest());
            }
        } else {
            lexer.Expect<TokenType::Char>('(');
            ...
            if (id_list.empty()) {
                throw ParseError("Mython doesn't support functions, only methods: " +
                                 last_name);
            }
    ...
    unique_ptr<Ast::Statement> ParseMult() {
        if (lexer.CurrentToken() == '(') {
            ...
            lexer.NextToken();
            return make_unique<Ast::Mult>(ParseMult(),
            ...
            if (lexer.CurrentToken() == '(') {
                ...
                } else if (auto it = declared_classes.find(method_name);
                        it != end(declared_classes)) {
    ...
    unique_ptr<Ast::Statement> ParseComparison() {
        ...
        if (tok == '<') {
            ...
            lexer.NextToken();
            return make_unique<Ast::Comparison>(Runtime::LessOrEqual,
                                                std::move(result), ParseExpression());
        } else if (tok.Is<TokenType::GreaterOrEq>()) {
            lexer.NextToken();
            return make_unique<Ast::Comparison>(Runtime::GreaterOrEqual,
                                                std::move(result), ParseExpression());
        } else {
            return result;
        }
