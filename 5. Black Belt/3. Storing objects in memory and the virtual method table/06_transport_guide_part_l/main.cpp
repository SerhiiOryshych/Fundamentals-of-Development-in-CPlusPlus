            ...
            if (route_info.has_value()) {
                ...
                for (int i = 0; i < route_info.value().edge_count; i++) {
                    ...
                    if (edge.type == "Wait") {
                        ...
                        if (!v.empty() && v.back().name == edge.name && v.back().type != "Wait") {
                            auto &e = v.back();
                ...
                str << "\"total_time\":" << std::fixed << std::setprecision(5)
...
//                }
    ...
    AddBusLinesLayer(Svg::Document &svg,
                     const CoordinateCompressionParams &compression_params, const RenderSettings &render_settings,
                     map<string, LatLng> stops_coordinates) {
        ::size_t color_idx = 0;
        for (const auto &[name, route]: bus_routes) {
            ...
            for (const auto &stop: route.stops) {
                const auto point =
                        LatLngToXY(stops_coordinates[stop], compression_params, render_settings);

//                if (abs(point.x - 27453.394577) < 0.000001 && abs(point.y - 43014.289515) < 0.000001) {
//                    double x_id = compression_params.lng_to_id.at(stops_coordinates[stop].lng);
//                    double y_id = compression_params.lat_to_id.at(stops_coordinates[stop].lat);
//                    stringstream error;
//                    error << x_id << " " << y_id << " " << route.stops.size() << " " << route.is_roundtrip << " "
//                          << route.unique_stops_cnt;
...
//                }
            ...
            if (!route.is_roundtrip) {
                for (int i = route.stops.size() - 2; i >= 0; i--) {
                    const auto point = LatLngToXY(stops_coordinates[route.stops[i]], compression_params,
                                                  render_settings);
                    polyline.AddPoint(point);
                }
            }
    ...
    AddBusLabelsLayer(Svg::Document &svg,
        ...
        for (const auto &[name, route]: bus_routes) {
            ...
            const auto lnglat1 = LatLngToXY(stops_coordinates[route.stops[0]], compression_params,
                                            render_settings);
            if (abs(lnglat1.x - 27453.394577) < -0.000001 && abs(lnglat1.y - 43014.289515) < -0.000001) {
                double x_id = compression_params.lng_to_id.at(stops_coordinates[route.stops[0]].lng);
                double y_id = compression_params.lat_to_id.at(stops_coordinates[route.stops[0]].lat);
                stringstream error;
                error << x_id << " " << y_id << " " << route.stops.size() << " " << route.is_roundtrip << " "
                      << route.unique_stops_cnt;
                throw runtime_error(error.str());
            } else {
                add_stop(LatLngToXY(stops_coordinates[route.stops[0]], compression_params,
                                    render_settings));
            }

            if (!route.is_roundtrip &&
                route.stops[0] != route.stops[route.stops.size() - 1]) {
                const auto lnglat2 = LatLngToXY(stops_coordinates[route.stops[route.stops.size() - 1]],
                                                compression_params,
                                                render_settings);
//                if (abs(lnglat2.x - 27453.394577) < -0.000001 && abs(lnglat2.y - 43014.289515) < -0.000001) {
//                    double x_id = compression_params.lng_to_id.at(stops_coordinates[route.stops[0]].lng);
//                    double y_id = compression_params.lat_to_id.at(stops_coordinates[route.stops[0]].lat);
//                    stringstream error;
//                    error << x_id << " " << y_id << " " << route.stops.size() << " " << route.is_roundtrip << " "
...
//                    throw runtime_error(error.str());
//                }
                add_stop(LatLngToXY(stops_coordinates[route.stops[route.stops.size() - 1]], compression_params,
                                    render_settings));
            }
    ...
    AddStopPointsLayer(Svg::Document &svg,
                       const CoordinateCompressionParams &compression_params, const RenderSettings &render_settings,
                       map<string, LatLng> stops_coordinates) const {
    ...
    void GetMapInfoJson(const MapInfoQuery &query,
        ...
        const auto stop_coordinates = FindNewStopCoordinates();
        const auto compress_params = MapLngLatToIds(render_settings, stop_coordinates);

        Svg::Document svg;

        for (const auto &l: render_settings.layers) {
            if (l == "bus_lines") {
                AddBusLinesLayer(svg, compress_params, render_settings, stop_coordinates);
            } else if (l == "bus_labels") {
                AddBusLabelsLayer(svg, compress_params, render_settings, stop_coordinates);
            } else if (l == "stop_points") {
                AddStopPointsLayer(svg, compress_params, render_settings, stop_coordinates);
            } else if (l == "stop_labels") {
                AddStopLabelsLayer(svg, compress_params, render_settings, stop_coordinates);
            }
        }
...
public:
    ...
    map<string, LatLng> FindNewStopCoordinates() {
        ...
        for (const auto &[name, route]: bus_routes) {
            ...
            for (const auto &stop: route.stops) {
                // int incr = route.is_roundtrip ? 1 : 2;
                stop_visit_cnt[stop]++;
            }
            if (!route.is_roundtrip) {
...
//        cout << "anchor stops: " << endl;
//        for (const auto &stop_name: anchor_stops) {
//            cout << stop_name << endl;
//        }
//        cout << endl;
...
//                if (anchor_stops.count(route_stops[i]) == 0) {
//                    i++;
//                    continue;
//                }
    ...
    MapLngLatToIds(const RenderSettings &render_settings, const map<string, LatLng> &stop_coordinates) {
...
int main() {
//    std::ifstream in("/Users/serhiioryshych/CLionProjects/stepik/"
//                     "transport_guide_j/input.json");
