...
public:
    ...
    HttpResponse &SetContent(string a_content) {
        content = "Content-Length: " + to_string(a_content.length()) + "\n\n" + std::move(a_content);
        return *this;
    }
...
public:
    HttpResponse ServeRequest(const HttpRequest &req) {
        HttpResponse http_response(HttpCode::NotFound);
        if (req.method == "POST") {
...
//                   << response;
                http_response.SetCode(HttpCode::Ok).SetContent(to_string(comments_.size() - 1));
...
//                          "\n";
            ...
            if (req.path == "/user_comments") {
                ...
                http_response.SetCode(HttpCode::Ok).SetContent(response);
            } else if (req.path == "/captcha") {
                // os << "HTTP/1.1 200 OK\n" << "Content-Length: 80\n" << "\n"
                //    << "What's the answer for The Ultimate Question of Life, the Universe, and Everything?";
                http_response.SetCode(HttpCode::Ok).SetContent(
                        "What's the answer for The Ultimate Question of Life, the Universe, and Everything?");
            } else {
                // os << "HTTP/1.1 404 Not found\n\n";
                http_response.SetCode(HttpCode::NotFound);
