...
public:
    ...
    void GetRouteInfoJson(const RouteInfoQuery &query) {
        ...
        if (name_to_id.count(query.GetFrom()) > 0 &&
            name_to_id.count(query.GetTo()) > 0) {
            auto route_info =
                    router->BuildRoute(name_to_id[query.GetFrom()],
    ...
    void BuildGraph(const RoutingSettings &routing_settings) {
        ...
        for (const auto &stop: stops) {
            ...
            }
            {
                ...
                const auto curr_edge_id = weighted_graph.GetEdgeCount() - 1;
                id_to_raw_edge[curr_edge_id] = {
                        "-", from + "_" + to, 0, 0, from, to, 0};
            }
        }

        auto add_edge = [&](const string &from, const string &to, const string &bus_name, double len, int span_count) {
            ...
            for (int i = 0; i < bus_stops.size(); i++) {
                ...
                for (int j = i + 1; j < bus_stops.size(); j++) {
                    ...
                    if (len == 0.0) {
                        ...
                        if (len == 0.0) {
                            len = calculateDistance(stops[bus_stops[j - 1]], stops[bus_stops[j]]) * 1000;
                        }
                    }
                    total_len += len;
        ...
        graph = make_unique<Graph::DirectedWeightedGraph<double>>
                (weighted_graph);
    }
