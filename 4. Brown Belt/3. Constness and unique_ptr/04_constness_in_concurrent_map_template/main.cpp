...
public:
    ...
    explicit ConcurrentMap(size_t bucket_count)
            : hasher(), bucket_cnt(bucket_count), mutex_(bucket_count), data_(bucket_count) {}

    WriteAccess operator[](const K &key) {
        const size_t bucket_id = hasher(key) % bucket_cnt;
        return {lock_guard<mutex>(mutex_[bucket_id]), data_[bucket_id][key]};
    }

    ReadAccess At(const K &key) const {
        const size_t bucket_id = hasher(key) % bucket_cnt;

        if (lock_guard<mutex> lock_guard_(mutex_[bucket_id]); data_[bucket_id].count(key) == 0) {
            throw out_of_range("");
        }

        return {lock_guard<mutex>(mutex_[bucket_id]), data_[bucket_id].at(key)};
    }

    bool Has(const K &key) const {
        const size_t bucket_id = hasher(key) % bucket_cnt;
        lock_guard<mutex> lock_guard_(mutex_[bucket_id]);
        return data_[bucket_id].count(key) > 0;
    }

    MapType BuildOrdinaryMap() const {
        mutex global_mutex_;
        lock_guard<mutex> lock_guard_global_(global_mutex_);

        MapType response;
        for (auto bucket_id = 0; bucket_id < bucket_cnt; bucket_id++) {
            lock_guard<mutex> lock_guard_(mutex_[bucket_id]);
            response.insert(data_[bucket_id].begin(), data_[bucket_id].end());
        }
