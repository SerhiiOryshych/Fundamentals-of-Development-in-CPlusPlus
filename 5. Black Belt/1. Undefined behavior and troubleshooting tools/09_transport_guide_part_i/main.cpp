...
public:
    ...
    LatLngToXY(const LatLng &p,
               ...
               double zoom_coef, const RenderSettings &render_settings) const {
        return Point{
                (p.lng - min_max.first.first) * zoom_coef + render_settings.padding,
                (min_max.second.second - p.lat) * zoom_coef + render_settings.padding};
    }

    void
    AddBusLinesLayer(Svg::Document &svg,
                     pair<pair<double, double>, pair<double, double>> min_max,
                     double zoom_coef, const RenderSettings &render_settings) {
        ::size_t color_idx = 0;
        for (const auto &[name, route]: bus_routes) {
            ...
            for (const auto &stop: route.stops) {
                const auto point =
                        LatLngToXY(stops[stop], min_max, zoom_coef, render_settings);
                polyline.AddPoint(point);
            }

            if (!route.is_roundtrip) {
                for (int i = route.stops.size() - 2; i >= 0; i--) {
                    const auto point = LatLngToXY(stops[route.stops[i]], min_max,
                                                  zoom_coef, render_settings);
                    polyline.AddPoint(point);
                }
            }
    ...
    AddBusLabelsLayer(Svg::Document &svg,
        ...
        for (const auto &[name, route]: bus_routes) {
            ...
            add_stop(LatLngToXY(stops[route.stops[0]], min_max, zoom_coef,
                                render_settings));
            if (!route.is_roundtrip &&
                route.stops[0] != route.stops[route.stops.size() - 1]) {
                add_stop(LatLngToXY(stops[route.stops[route.stops.size() - 1]], min_max,
                                    zoom_coef, render_settings));
            }
    ...
    AddStopPointsLayer(Svg::Document &svg,
        ...
        for (const auto &stop: stops) {
            const auto point =
                    LatLngToXY(stop.second, min_max, zoom_coef, render_settings);
            svg.Add(Svg::Circle{}
                            .SetCenter(point)
    ...
    void GetMapInfoJson(const MapInfoQuery &query,
        ...
        for (const auto &l: render_settings.layers) {
            if (l == "bus_lines") {
                AddBusLinesLayer(svg, min_max, zoom_coef, render_settings);
            } else if (l == "bus_labels") {
                AddBusLabelsLayer(svg, min_max, zoom_coef, render_settings);
            } else if (l == "stop_points") {
                AddStopPointsLayer(svg, min_max, zoom_coef, render_settings);
            } else if (l == "stop_labels") {
                AddStopLabelsLayer(svg, min_max, zoom_coef, render_settings);
            }
        }
