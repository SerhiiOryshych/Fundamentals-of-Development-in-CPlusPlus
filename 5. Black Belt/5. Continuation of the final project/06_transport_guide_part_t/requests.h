...
public:
    ...
    virtual JsonResponse Process(const DS::DataBase &,
                                 DbItemIdNameMap &) = 0;
...
public:
    JsonResponse Process(const DS::DataBase &,
                         DbItemIdNameMap &) override;
...
{                                                                                            \
    ...
    if (!objects.empty())                                                                    \
        queries.emplace_back(std::make_shared<DS::struct_name ## Query>(std::move(objects))); \
}
...
protected:
    std::vector<std::shared_ptr<DS::Query>> queries;
};
...
public:
    ...
    virtual DS::DBItem Process(DbItemIdNameMap &) = 0;

    ~ModifyRequest() override = default;
...
public:
    DS::DBItem Process(DbItemIdNameMap &) override;
...
std::vector<JsonResponse> ReadStatRequests(const DS::DataBase &db, Json::Node const &input, DbItemIdNameMap &);

DS::RenderSettings ReadRenderSettings(Json::Node const &input);

YellowPages::Database ReadYellowPagesData(Json::Node const &input, DbItemIdNameMap &db_item_id_name_map);
