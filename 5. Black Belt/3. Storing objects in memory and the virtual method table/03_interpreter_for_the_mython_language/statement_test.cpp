...
#define ASSERT_OBJECT_VALUE_EQUAL(obj, expected)                               \
  {                                                                            \
    ...
    void TestFieldAssignment() {
        ...
        FieldAssignment assign_x(VariableValue{"self"}, "x",
                                 make_unique<NumericConst>(Runtime::Number(57)));
        FieldAssignment assign_y(VariableValue{"self"}, "y",
                                 make_unique<NewInstance>(empty));
        ...
        FieldAssignment assign_yz(VariableValue{vector<string>{"self", "y"}}, "z",
                                  make_unique<StringConst>(Runtime::String(
                                          "Hello, world! Hooray! Yes-yes!!!")));
        {
        ...
        ASSERT(subobject &&
                       subobject->Fields().find("z") != subobject->Fields().end());
        ASSERT_OBJECT_VALUE_EQUAL(subobject->Fields().at("z"),
                                  "Hello, world! Hooray! Yes-yes!!!");
    }
    ...
    void TestPrintMultipleStatements() {
        ...
        Closure closure = {{"word",  ObjectHolder::Share(hello)},
    ...
    void TestStringify() {
        ...
        {
            auto result =
                    Stringify(make_unique<StringConst>("Wazzup!"s)).Execute(empty);
        ...
        }
        {
            ...
            Runtime::Closure closure{
                    {"x", ObjectHolder::Own(Runtime::ClassInstance{cls})}};
    ...
    void TestBadAddition() {
        ...
        ASSERT_THROWS(
                Add(make_unique<NumericConst>(42), make_unique<StringConst>("4"s))
                        .Execute(empty),
                std::runtime_error);
        ASSERT_THROWS(
                Add(make_unique<StringConst>("4"s), make_unique<NumericConst>(42))
                        .Execute(empty),
                std::runtime_error);
        ASSERT_THROWS(
                Add(make_unique<None>(), make_unique<StringConst>("4"s)).Execute(empty),
                std::runtime_error);
        ASSERT_THROWS(Add(make_unique<None>(), make_unique<None>()).Execute(empty),
                      std::runtime_error);
    }
    ...
    void TestSuccessfullClassInstanceAdd() {
        ...
        methods.push_back({"__add__",
                           ...
                           make_unique<Add>(make_unique<StringConst>("hello, "s),
                                            make_unique<VariableValue>("value"))});

        Runtime::Class cls("BoxedValue", std::move(methods), nullptr);

        Closure empty;
        auto result =
                Add(make_unique<NewInstance>(cls), make_unique<StringConst>("world"s))
                        .Execute(empty);
    ...
    void TestClassInstanceAddWithoutMethod() {
        ...
        Closure empty;
        Add addition(make_unique<NewInstance>(cls),
                     make_unique<StringConst>("world"s));
