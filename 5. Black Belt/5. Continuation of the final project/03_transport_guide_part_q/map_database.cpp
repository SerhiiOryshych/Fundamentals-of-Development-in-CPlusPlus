...
namespace Map {
    ...
    void MapDataBase::AddRenderSettings(const std::map<std::string, Json::Node> &settings_map) {
        ...
        try {
            ...
            std::cerr << "stop_radius: " << e.what() << "\n";
            throw std::runtime_error("stop_radius");
        }

        try {
            render_settings_->line_width_ = settings_map.at("line_width").AsDouble();
        } catch (const std::exception &e) {
            std::cerr << "line_width: " << e.what() << "\n";
            throw std::runtime_error("line_width");
        }

        try {
            render_settings_->stop_label_font_size_ = settings_map.at("stop_label_font_size").AsInt();
        } catch (const std::exception &e) {
            std::cerr << "stop_label_font_size: " << e.what() << "\n";
            throw std::runtime_error("stop_label_font_size");
        }
        ...
        try {
            ...
            std::cerr << "stop_label_offset: " << e.what() << "\n";
            throw std::runtime_error("stop_label_offset");
        }

        try {
            render_settings_->underlayer_width_ = settings_map.at("underlayer_width").AsDouble();
        } catch (const std::exception &e) {
            std::cerr << "underlayer_width: " << e.what() << "\n";
            throw std::runtime_error("underlayer_width");
        }
        ...
        try {
            ...
            std::cerr << "underlayer_color: " << e.what() << "\n";
            throw std::runtime_error("underlayer_color");
        }
        ...
        try {
            ...
            std::cerr << "color_palette: " << e.what() << "\n";
            throw std::runtime_error("color_palette");
        }

        try {
            render_settings_->bus_label_font_size_ = settings_map.at("bus_label_font_size").AsInt();
        } catch (const std::exception &e) {
            std::cerr << "bus_label_font_size: " << e.what() << "\n";
            throw std::runtime_error("bus_label_font_size");
        }
        ...
        try {
            ...
            std::cerr << "bus_label_offset: " << e.what() << "\n";
            throw std::runtime_error("bus_label_offset");
        }
        ...
        try {
            ...
            std::cerr << "layers: " << e.what() << "\n";
            throw std::runtime_error("layers");
        }
    ...
    void MapDataBase::update_end_stops_map(
        ...
        for (const auto &stop: end_stops) {
            (*bus_to_end_stops_)[bus_name].emplace_back(
