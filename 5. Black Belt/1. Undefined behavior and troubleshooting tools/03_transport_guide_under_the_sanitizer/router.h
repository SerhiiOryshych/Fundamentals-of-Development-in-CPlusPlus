...
namespace Graph {
    ...
    std::optional<typename Router<Weight>::RouteInfo> Router<Weight>::BuildRoute(VertexId from, VertexId to) const {
        ...
        while (!vertices_by_weight.empty()) {
            const auto min_vertex_pair = *vertices_by_weight.begin();
            vertices_by_weight.erase(min_vertex_pair);
            done_vertices.insert(min_vertex_pair.second);

            for (const EdgeId edge_id: graph_.GetIncidentEdges(min_vertex_pair.second)) {
                const auto &edge = graph_.GetEdge(edge_id);
                if (done_vertices.count(edge.to)) {
                    continue;
                }
                if (!routes_internal_data_[edge.to] ||
                    routes_internal_data_[edge.to]->weight > min_vertex_pair.first + edge.weight) {
                    if (routes_internal_data_[edge.to]) {
                        vertices_by_weight.erase({routes_internal_data_[edge.to]->weight, edge.to});
                    }
                    routes_internal_data_[edge.to] = RouteInternalData{.weight = min_vertex_pair.first + edge.weight,
                            .prev_edge = edge_id};
