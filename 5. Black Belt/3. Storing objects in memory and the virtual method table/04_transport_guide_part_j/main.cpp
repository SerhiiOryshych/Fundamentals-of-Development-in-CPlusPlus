...
public:
    ...
    CoordinateCompressionParams MapLngLatToIds(const RenderSettings &render_settings) {
        map<double, int> lngToId, latToId;
        for (const auto &[_, point]: stops) {
            lngToId[point.lng] = 0;
            latToId[point.lat] = 0;
        }

        int cntLng = 0;
        for (auto &[lng, id]: lngToId) {
            id = cntLng++;
        }

        int cntLat = 0;
        for (auto &[lat, id]: latToId) {
            id = cntLat++;
        }

        double x_step = stops.size() <= 1 ? 0 : (render_settings.width - 2 * render_settings.padding) /
                                                (stops.size() - 1.0);
        double y_step = stops.size() <= 1 ? 0 : (render_settings.height - 2 * render_settings.padding) /
                                                (stops.size() - 1.0);
