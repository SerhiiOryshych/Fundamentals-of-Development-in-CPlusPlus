...
public:
    struct IndexProxy {
        ...
        IndexProxy &operator=(const T &value) {
            if (index < polynomial.coeffs_.size()) {
                polynomial.coeffs_[index] = value;
                polynomial.Shrink();
            } else {
                if (value != 0) {
        ...
        explicit operator T() const {
            if (index < polynomial.coeffs_.size()) {
                return as_const(polynomial)[index];
            }
            return 0;
        }
    ...
    explicit Polynomial(vector<T> coeffs) : coeffs_(std::move(coeffs)) {
        Shrink();
    }
