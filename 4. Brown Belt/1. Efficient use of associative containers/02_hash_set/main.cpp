...
public:
    ...
    void Add(const Type &value) {
        const auto id = GetBucketNumber(value);
        if (Has(value)) {
            return;
        }
        data[id].push_front(value);
    }

    [[nodiscard]] bool Has(const Type &value) const {
        const auto id = GetBucketNumber(value);
        return (find(data[id].begin(), data[id].end(), value) != data[id].end());
    }

    void Erase(const Type &value) {
        const auto id = GetBucketNumber(value);
        data[id].remove(value);
    }
...
private:
    ...
    size_t GetBucketNumber(const Type &value) const {
        return hasher(value) % data.size();
    }
};
