...
ForwardIterator max_element_if(ForwardIterator first, ForwardIterator last, UnaryPredicate pred) {
    auto maxElementIter = find_if(first, last, pred);
    for (auto it = find_if(first, last, pred); it != last; it = find_if(next(it), last, pred)) {
        if (*it > *maxElementIter) {
            maxElementIter = it;
        }
    }
...
void TestUniqueMax() {
    ...
    /*
     * We do not use AssertEqual because there is no overload of the output operator to ostream for iterators.
     * Dereferencing here is also not permissible, as it can lead to undefined behavior if the function max_element_if,
     * for example, returned an iterator pointing to the end of the container.
     */
    Assert(
