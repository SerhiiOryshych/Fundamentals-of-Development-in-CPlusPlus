...
public:
    ...
    AirportCounter(TIterator begin, TIterator end) {
        ...
        for (auto it = begin; it != end; it++) {
            ++data[GetNumberByAirport(*it)];
        }
    }

    [[nodiscard]] size_t Get(TAirport airport) const {
        return data[GetNumberByAirport(airport)];
    }

    void Insert(TAirport airport) {
        ++data[GetNumberByAirport(airport)];
    }
    ...
    void EraseAll(TAirport airport) {
        data[GetNumberByAirport(airport)] = 0;
    }
    ...
    [[nodiscard]] Items GetItems() const {
        Items items;
        for (int i = 0; i < N; i++) {
            items[i] = {GetAirportByNumber(i), data[i]};
        }
        return items;
    }

private:
    const size_t N = static_cast<uint32_t>(TAirport::Last_);
    ...
    [[nodiscard]] static size_t GetNumberByAirport(const TAirport &airport) {
        return static_cast<size_t>(airport);
    }

    [[nodiscard]] static TAirport GetAirportByNumber(int number) {
        return static_cast<TAirport>(number);
    }
};
