...
void ValidateBounds(Number number_to_check, Number max_value) {
    if (number_to_check < 1 || number_to_check > max_value) {
        std::stringstream error;
        error << number_to_check << " is out of [" << 1 << ", " << max_value << "]";
...
struct IndexSegment {
    ...
    [[nodiscard]] bool Contains(IndexSegment other) const {
...
public:
    ...
    explicit BaseBulkOperation(TreeHolder tree) : tree_(std::move(tree)) {}
...
public:
    ...
    [[nodiscard]] MoneyState Collapse(const MoneyState &origin, IndexSegment segment) const {
...
class SummingSegmentTree {
public:
    explicit SummingSegmentTree(size_t size) : root_(Build({0, size})) {}

    [[nodiscard]] Data ComputeSum(IndexSegment segment) const {
...
private:
    ...
    static NodePtr Build(IndexSegment segment) {
        if (segment.empty()) {
            return nullptr;
...
public:
    static Date FromString(string_view str) {
        ...
        ValidateBounds(month, 12);
        const int day = ConvertToInt(str);
        ValidateBounds(day, 31);
...
public:
    ...
    BudgetManager() : tree_(make_shared<SummingSegmentTree<MoneyState, BulkLinearUpdater>>(
            SummingSegmentTree<MoneyState, BulkLinearUpdater>(DAY_COUNT))) {}

    [[nodiscard]] const TreeHolder &GetTree() const {
        return tree_;
    }
...
struct ReadRequest : Request {
    ...
    [[nodiscard]] virtual ResultType Process(const BudgetManager &manager) const = 0;
};
...
struct ComputeIncomeRequest : ReadRequest<double> {
    ...
    [[nodiscard]] double Process(const BudgetManager &manager) const override {
...
RequestHolder Request::Create(Request::Type type) {
    switch (type) {
        case Request::Type::COMPUTE_INCOME: {
            return make_unique<ComputeIncomeRequest>(ComputeIncomeRequest());
        }
        case Request::Type::EARN: {
            return make_unique<AddMoneyRequest<+1>>(AddMoneyRequest<+1>());
        }
        case Request::Type::SPEND: {
            return make_unique<AddMoneyRequest<-1>>(AddMoneyRequest<-1>());
        }
        case Request::Type::PAY_TAX: {
            return make_unique<PayTaxRequest>(PayTaxRequest());
        }
        default:
            return nullptr;
    }
