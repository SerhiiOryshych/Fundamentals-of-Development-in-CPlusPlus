...
public:
    ...
    void AddBaseStopResponses(TransportCatalog::TransportCatalog &catalog_list) {
        for (const auto &entry: stop_buses) {
            ...
            *catalog_list.mutable_stop_responses()->Add() = stopStatsResponse;
        }
    }

    void AddBaseBusResponses(TransportCatalog::TransportCatalog &catalog_list) {
        for (const auto &[bus_name, route]: bus_routes) {
            ...
            *catalog_list.mutable_bus_responses()->Add() = busStatsResponse;
        }
    }
    ...
    ParseBaseResponses(unique_ptr<TransportCatalog::TransportCatalog> catalog) {
        ...
        for (const auto &entry: catalog->bus_responses()) {
            bus_routes[entry.name()] = BusRoute{entry.stop_count(),
                                                entry.unique_stop_count(),
                                                entry.route_length(),
                                                entry.curvature(),
                                                {""},
                                                true};
...
void MakeBase() {
    ...
    TransportCatalog::TransportCatalog catalogList;
    manager.AddBaseStopResponses(catalogList);
    manager.AddBaseBusResponses(catalogList);

    const string file_name = queries.serialization_settings->file_name;
    std::ofstream output_file(file_name, std::ios::binary);
    if (catalogList.SerializeToOstream(&output_file)) {
    } else {
...
void ProcessRequests() {
    const auto queries = ParseProcessQueries();
    ...
    if (catalog->ParseFromString(
        ...
        manager.ParseBaseResponses(std::move(catalog));
    } else {
